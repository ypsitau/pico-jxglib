...
void tud_video_frame_xfer_complete_cb(uint_fast8_t ctl_idx, uint_fast8_t stm_idx)
{
	...
	Video* pVideo = Controller::GetInterface_Video(0);
	if (pVideo) pVideo->On_frame_xfer_complete(ctl_idx, stm_idx);
}
...
int tud_video_commit_cb(uint_fast8_t ctl_idx, uint_fast8_t stm_idx, const video_probe_and_commit_control_t* parameters)
{
	using namespace jxglib::USBDevice;
	Video* pVideo = Controller::GetInterface_Video(0);
	return pVideo? pVideo->On_commit(ctl_idx, stm_idx, parameters) : VIDEO_ERROR_NONE;
}
