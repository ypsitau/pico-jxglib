...
#include "test-PIO-blink.pio.h"

using namespace jxglib;

int main()
{
	PIOContext context_blink(blink_program);
	context_blink.ClaimResource();
	//context_blink.ClaimResource(GPIO10, 1, true);
	PIOIf::Config cfg(blink_program_get_default_config(context_blink.offset));
	context_blink.pio.gpio_init(GPIO10);
	context_blink.sm.set_consecutive_pindirs_out(GPIO10, 1);
	cfg.set_set_pins(GPIO10, 1);
	context_blink.Init(cfg);
	context_blink.sm.set_enabled(true);
	int freq = 10;
	//printf("Blinking pin %d at %d Hz\n", pin, freq);
	// PIO counter program takes 3 more cycles in total than we pass as
	// input (wait for n + 1; mov; jmp)
	context_blink.sm.put((clock_get_hz(clk_sys) / (2 * freq)) - 3);
}
