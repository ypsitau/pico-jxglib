...
bool DS323x::DoGet(DateTime* pDt)
{
	...
	::i2c_write_blocking(i2c_, addr_, &reg, sizeof(reg), true); // no-stop condition
	uint8_t buf[7] = {};
	::i2c_read_blocking(i2c_, addr_, buf, sizeof(buf), false);
...
bool DS323x::DoSet(const DateTime &dt)
{
	...
	::i2c_write_blocking(i2c_, addr_, buf, sizeof(buf), false);
	return true;
}
